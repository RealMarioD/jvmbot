...
exports.run = (client, message, args) => {
    ...
    switch(action) {
        ...
        default:
            message.reply('nincs ilyen RPG funkció!');
            break;
    }
    ...
    function adventure(revived) {
        ...
        const foundApple = [giveRandom(10000), giveRandom(u.rpg.level)];
        const foundGuitar = [giveRandom(1000000), giveRandom(u.rpg.level)];
        ...
        switch(where) {
            case 'inv':
                Object.keys(u.rpg.items).map(key => ({
                    ...
                    if(data.value.itemData.name.toLowerCase() == name) {
                        returnValue = data;
                    }
                });
                break;
            ...
            default:
                Object.keys(rpgData.shopItems).map(key => ({
                    ...
                    if(data.value.name.toLowerCase() == name) {
                        returnValue = data;
                    }
                });
                break;
    ...
    function shop(item, boughtAmount) {
        ...
        if(!boughtAmount) {
            ...
            if(u.money >= total) {
                ...
                message.reply(`nincs elég pénzed hogy vehess \`${boughtAmount}\`db \`${selectedItem.value.name}\` itemet!`);
            }
        }
    }
    ...
    function showInventory() {
        ...
        for(const item in invItems) {
            invEmbed.description += `${invItems[item][0]} - ${invItems[item][1]}db\n`;
        }
