...
exports.run = async (client, message, args) => {
    ...
    const playOptions = (originalMessage, passedMsg) => {
        ...
        if(countCards(gameCards['player'], 'player') == 21 && gameCards['player'].length == 2) {
            pushCards('dealer');
            if(countCards(gameCards['dealer'], 'dealer') == 21) {
        ...
        .then(collected => {
            if(!['hit', 'stay'].includes(collected.first().content.toLowerCase())) {
                ...
                return selectMove(collected.first().content.toLowerCase(), originalMessage, passedMsg);
            }
        }).catch(() => {
    ...
    function pushCards(user) {
        let toPush;
        do {
            toPush = `${cards[giveRandom(13)]} ${suits[giveRandom(3)]}`;
