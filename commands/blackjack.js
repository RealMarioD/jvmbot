...
exports.run = async (client, message, args) => {
    ...
    if(args.length == 0) message.channel.send("> ❌ Nem adtál meg tétet!");
        ...
        const selectMove = (choice, originalMessage) => {
            switch (choice) {
                ...
                case 'stay':
                    ...
                    let finalPlayerTotal = countCards(gameCards['player'], 'player');
                    let finalDealerTotal = countCards(gameCards['dealer'], 'dealer');
        ...
        let ownHand = determineHand(gameCards['player'], countCards(gameCards['player'], 'player'), `${message.author.username}`);
        let dealerHand = determineHand(gameCards['dealer'], countCards(gameCards['dealer'], 'dealer'), `Az osztó`);
